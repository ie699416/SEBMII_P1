...
//	configMAX_PRIORITIES - 4, NULL);
	...
	xTaskCreate(Chat_Task, "Chat_Task", 200, NULL,
	configMAX_PRIORITIES-2, NULL);
...
void Chat_Task(void *arg) {
	xEventGroupWaitBits(get_g_TERM1_events(), EVENT_CHAT_TASK_ON, pdTRUE, pdFALSE,
	portMAX_DELAY);
	UART1_putString(getClearScreen());
	...
	for (;;) {
		...
		xEventGroupWaitBits(get_g_TERM1_events(), EVENT_CHAR_SENT, pdTRUE, pdFALSE,
		portMAX_DELAY);

		xQueueReceive(get_g_UART1_mailbox(), &chatBffr[0], portMAX_DELAY);
