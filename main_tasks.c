...
void I2C_RTC_callback(I2C_Type *base, i2c_master_handle_t *handle,
	...
	if (status == kStatus_Success) {
		xResult = xEventGroupSetBitsFromISR(g_I2C_events,
		EVENT_I2C_RTC_TX_COMPLETE, &xHigherPriorityTaskWoken);
...
void I2C_EEPROM_callback(I2C_Type *base, i2c_master_handle_t *handle,
		status_t status, void *userData) {
	BaseType_t xHigherPriorityTaskWoken;
	BaseType_t xResult;
	...
	if (status == kStatus_Success) {
		xResult = xEventGroupSetBitsFromISR(g_I2C_events,
		EVENT_I2C_EEPROM_TX_COMPLETE, &xHigherPriorityTaskWoken);
		g_MasterCompletionFlag = true;
...
void Read_EEPROM(void * arg) {
	...
	i2c_master_handle_t g_I2C_EEPROM_handle;

	I2C_MasterTransferCreateHandle(I2C0_BASEADDR, &g_I2C_EEPROM_handle,
			I2C_EEPROM_callback, NULL);
	...
	I2C_MasterTransferNonBlocking(I2C0, &g_I2C_EEPROM_handle, &masterXfer);
	while (!g_MasterCompletionFlag) {

	}
	//Wait for transfer completed.
	//xEventGroupWaitBits(g_I2C_events, EVENT_I2C_EEPROM_TX_COMPLETE,pdTRUE, pdFALSE, portMAX_DELAY);
...
void Write_EEPROM(void * arg) {
	...
	I2C_MasterTransferNonBlocking(I2C0_BASEADDR, &g_I2C_EEPROM_handle,
			&masterXfer);
	while (!g_MasterCompletionFlag) {
...
void iReadRTC_task(void * arg) {
	...
	i2c_master_handle_t g_I2C_RTC_handle;

	I2C_MasterTransferCreateHandle(I2C0_BASEADDR, &g_I2C_RTC_handle,
			I2C_EEPROM_callback, NULL);
	...
	I2C_MasterTransferNonBlocking(I2C0, &g_I2C_RTC_handle, &masterXfer);
	xEventGroupWaitBits(g_I2C_events, EVENT_I2C_RTC_TX_COMPLETE,
	pdTRUE, pdFALSE, portMAX_DELAY);
